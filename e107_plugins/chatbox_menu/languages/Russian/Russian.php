<?php
/*
 * e107 website system
 *
 * Copyright (C) 2008-2009 e107 Inc (e107.org)
 * Released under the terms and conditions of the
 * GNU General Public License (http://www.gnu.org/licenses/gpl.txt)
 *
 * $Source: /cvs_backup/e107_0.8/e107_plugins/chatbox_menu/languages/English/English.php,v $
 * $Revision  12 $
 * $Date 2019/02/03 $
 * $Author  net1313 $
 */
define("CHATBOX_L1", "Невозможно создать пост от имени зарегистрированного пользователя. Если вы, войдите в систему, чтобы опубликовать.");
define("CHATBOX_L3", "Вы должны авторизоваться. Пожалуйста, авторизуйтесь через блок входа, или [отсюда].");
define("CHATBOX_L3b", "Если у вас нет учетной записи, [зарегистрируйтесь]");
define("CHATBOX_L4", "Отправить");
define("CHATBOX_L5", "Сброс");
define("CHATBOX_L6", "[забанен админом]");
define("CHATBOX_L7", "Разбанить");
define("CHATBOX_L8", "Инфа");
define("CHATBOX_L9", "Забанить");
define("CHATBOX_L11", "Пока чат пуст");
define("CHATBOX_L12", "Смотреть все");
define("CHATBOX_L13", "модерировать");
define("CHATBOX_L14", "Смайлики");
define("CHATBOX_L15", "Передан слишком длинный пост, либо пустой");
define("CHATBOX_L17", "Дубликат поста");
define("CHATBOX_L18", "Модерация завершена");
define("CHATBOX_L19", "Можно постить через [x] секунд");
define("CHATBOX_L20", "Чат (все посты)");
define("CHATBOX_L22", "в");
define("CHATBOX_L24", "У вас недостаточно прав для просмотра этой страницы.");
define("CHATBOX_L25", "[ пост заблокирован админом ]");
define("LAN_CHATBOX_100", "Чатим тут");
